...
# 4.Try to convert date in to 3 variables. It may work.
# 5.Creating some new features may help.
...
set.seed(1111)
time.now = proc.time()
model.rf = train(country_destination ~., method = "rf", data = data.train, ntree = 20, do.trace = TRUE)
proc.time() - time.now
# save(model.rf, file = 'model.rf.1229.Rds')
